...
class FirestoreHistoryRepository(FirestoreRepoBase):
    ...
    def _create_artifact_sync(
        ...
        session_ref = self._db.collection("sessions").document(session_id)
        run_ref = session_ref.collection("runs").document(run_id)
        # Immutable durable ID: pre-generated by caller when uploading to GCS so
        # blob path {session}/{run}/{artifact_id}/{file} matches doc id.
        # Falls back to random for legacy callers.
        if isinstance(artifact_id, str) and artifact_id.strip():
            artifact_id = "".join(ch for ch in artifact_id.strip() if ch.isalnum() or ch in {"-", "_"})[:64]
        else:
            artifact_id = uuid.uuid4().hex[:12]
        artifact_ref = run_ref.collection("artifacts").document(artifact_id)
        meta = dict(metadata or {})
        if not meta.get("gcs_bucket") or not meta.get("gcs_blob"):
            logger.warning(
                "ARTIFACT_MISSING_DURABLE_META artifact=%s session=%s run=%s kind=%s "
                "bytes will not survive sandbox expiry",
                artifact_id,
                session_id,
                run_id,
                kind,
        ...
        def transactional_create(txn):
            ...
            payload: dict[str, Any] = {
                ...
                "createdAt": now,
                "metadata": meta,
                "durable": bool(meta.get("gcs_bucket") and meta.get("gcs_blob")),
            }
            if source_step_id:
    ...
    def _get_artifact_for_owner_sync(
        ...
        # Durable resolver: try direct session path, then task mirrors, then
        # collection-group. All sessions must resolve even when sandbox is long
        # gone and parent session doc was cleaned up.
        ...
        def _try_doc(ref) -> StoredArtifact | None:
            try:
                doc = ref.get()
            except Exception:
                logger.warning("ARTIFACT_DOC_READ_FAILED artifact=%s", artifact_id, exc_info=True)
                return None
            ...
            found_rid = str(data.get("runId") or rid or "")
            if not found_sid:
                return None
            return self._build_stored_artifact(found_sid, found_rid, doc.id, data)

        # 1. Direct session path - no index needed, preview pane fast path.
        if sid and rid:
            hit = _try_doc(
                self._db.collection("sessions")
                .document(sid)
        ...
        # 2. Task mirrors (written by _create_artifact_sync). Derive task_id
        # from session doc when available, else try sid as task_id fallback.
        task_ids: list[str] = []
        if sid:
            try:
                sdoc = self._db.collection("sessions").document(sid).get()
                sdata = sdoc.to_dict() or {} if getattr(sdoc, "exists", False) else {}
                tid = sdata.get("taskId")
        ...
        for task_id in task_ids:
            # users/{owner}/tasks/{task}/artifacts/{aid}
            hit = _try_doc(self._task_ref(owner_id, task_id).collection("artifacts").document(artifact_id))
            if hit:
                return hit
            # users/{owner}/tasks/{task}/runs/{rid}/artifacts/{aid} when rid known
            if rid:
                hit = _try_doc(
                    self._task_run_ref(owner_id, task_id, rid).collection("artifacts").document(artifact_id)
                )
                if hit:
                    return hit

        # 3. Collection-group fallback (needs firestore.indexes.json deployed).
        try:
            docs = (
            ...
            if self._is_missing_firestore_index_error(exc):
                logger.error(
                    "ARTIFACT_INDEX_MISSING artifact=%s deploy firestore:indexes "
                    "(artifacts ownerId+artifactId): %s",
                    artifact_id,
                    exc,
                )
            else:
                logger.warning(
                    "ARTIFACT_GROUP_LOOKUP_FAILED artifact=%s",
                    artifact_id,
                    exc_info=True,
                )
        logger.warning("ARTIFACT_DOC_NOT_FOUND artifact=%s owner=%s", artifact_id, owner_id)
        return None

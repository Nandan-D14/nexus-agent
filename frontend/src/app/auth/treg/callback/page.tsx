...
export default function TregCallbackPage() {
  ...
  const [exchangeMessage, setExchangeMessage] = useState("Connecting Treg...");
  ...
  const preflight = useMemo(() => {
    ...
    if (oauthError) {
      return {
        ...
        message: `Treg refused access: ${oauthError}`,
      };
    }
    if (!code || !state) {
      return {
  ...
  useEffect(() => {
    ...
    authenticatedFetch("/api/v1/auth/treg/exchange", {
      method: "POST",
      ...
      .then(async (response) => {
        ...
        setExchangeMessage("Treg connected!");
        if (window.opener) {
          window.opener.postMessage({ type: "treg_connected", connected: true }, window.location.origin);

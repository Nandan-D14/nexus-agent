...
export default function ExaCallbackPage() {
  ...
  const [exchangeMessage, setExchangeMessage] = useState("Connecting Exa...");
  ...
  const preflight = useMemo(() => {
    ...
    if (oauthError) {
      return {
        ...
        message: `Exa refused access: ${oauthError}`,
      };
    }
    if (!code || !state) {
      return {
  ...
  useEffect(() => {
    ...
    authenticatedFetch("/api/v1/auth/exa/exchange", {
      method: "POST",
      ...
      .then(async (response) => {
        ...
        setExchangeMessage("Exa connected!");
        if (window.opener) {
          window.opener.postMessage({ type: "exa_connected", connected: true }, window.location.origin);

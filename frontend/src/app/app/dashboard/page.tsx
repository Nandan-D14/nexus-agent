...
import { DEFAULT_PLAN_QUOTA } from "@/lib/message-types";
import { APP_HISTORY, APP_HOME, historySessionPath, sessionPath } from "@/lib/app-paths";
import {
  EMPTY_TOKEN_TOTALS,
...
import { useDeleteHistorySessionMutation } from "@/lib/queries/history";
import { useQuotaQuery } from "@/lib/queries/quota";
import { useActiveSessionsQuery } from "@/lib/queries/sessions";
...
export default function DashboardPage() {
  ...
  const usageQuery = useDashboardUsageQuery(30);
  const recentQuery = useDashboardSessionsQuery(12);
  const activeQuery = useActiveSessionsQuery();
  const quotaQuery = useQuotaQuery();
  const endSessionMutation = useDeleteHistorySessionMutation();
  const stats = statsQuery.data ?? null;
  const usage = usageQuery.data ?? [];
  const recentSessions = recentQuery.data ?? [];
  const activeSessions = activeQuery.data ?? [];
  const quota = quotaQuery.data ?? (quotaQuery.isError ? DEFAULT_PLAN_QUOTA : null);
  ...
  const error =
    ...
    (usageQuery.error instanceof Error ? usageQuery.error.message : null) ||
    (recentQuery.error instanceof Error ? recentQuery.error.message : null) ||
  ...
  const handleEndSession = async (sessionId: string) => {
    ...
    try {
      ...
      void queryClient.invalidateQueries({ queryKey: queryKeys.dashboard.usage(30) });
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 px-4 pb-20 pt-4 text-foreground md:px-8">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="font-serif text-3xl leading-none tracking-tight text-zinc-950 sm:text-4xl dark:text-white">
            Usage
          </h1>
          <p className="mt-2 max-w-2xl text-sm text-zinc-500 dark:text-zinc-400">
            {quota
              ? `${quota.plan_name || "Plan"} · ${formatCompactNumber(quota.used)} of ${formatCompactNumber(quota.limit)} credits`
              : "Credits and quota for your workspace."}
          </p>
        </div>
        <button
        ...
        >
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex items-center gap-4">
              <p className="text-sm text-zinc-500">{usagePercent}% used</p>
              <button
                type="button"
                onClick={() => openSettings("billing")}
                className="inline-flex items-center justify-center rounded-full border border-zinc-200 bg-white px-4 py-2 text-sm font-medium text-zinc-900 hover:bg-zinc-50 dark:border-white/15 dark:bg-white/10 dark:text-white dark:hover:bg-white/15"
              >
                Manage billing
              </button>
            </div>
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <section className="rounded-3xl border border-zinc-200 bg-white p-6 dark:border-white/5 dark:bg-white/[0.02]">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link href={APP_HISTORY} className="text-sm text-cyan-600">View all</Link>
            </div>
                  ...
                  {recentSessions.map((s) => (
                    <tr key={s.session_id}>
                      <td className="py-3 px-2">
                        <Link href={historySessionPath(s.session_id)} className="font-medium hover:text-cyan-600">{s.title}</Link>
              ...
              {activeSessions.map((s) => (
                <div key={s.session_id} className="p-4 rounded-2xl bg-zinc-50 dark:bg-white/5 border border-zinc-100 dark:border-white/5">
                  ...
                  <div className="flex gap-2 mt-4">
                    <Link href={sessionPath(s.session_id)} className="flex-1 text-center py-2 bg-zinc-950 text-white dark:bg-white dark:text-black rounded-full text-xs font-medium">Resume</Link>
                    <button
                      onClick={() => void handleEndSession(s.session_id)}
                      disabled={endingSessionId === s.session_id}
                      className="px-4 py-2 border border-red-500/20 text-red-500 rounded-full text-xs font-medium"
                    >
                      End
                    </button>
                  </div>

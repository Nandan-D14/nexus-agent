...
gcloud secrets create task-worker-auth-token --project="${PROJECT_ID}" 2>/dev/null || true
...
echo "  echo -n 'YOUR_WORKER_TOKEN' | gcloud secrets versions add task-worker-auth-token --data-file=- --project=${PROJECT_ID}"
